...
public class PieRadarChartRenderer extends DataRenderer {

    protected static final int mOuterSpaceAlpha = 80;
    protected PieRadarChart mChart;
    ...
    public PieRadarChartRenderer(PieRadarChart chart, ChartAnimator animator,
    ...
    public void drawData(Canvas c) {
        ...
        for (IPieDataSet set : pieData.getDataSets()) {
            ...
            if (set.isVisible() && set.getEntryCount() > 0) {
                drawOuterDataSet(c, set);
                drawInnerDataSet(c, set);
            }
        }
    }
    ...
    protected void drawInnerDataSet(Canvas c, IPieDataSet dataSet) {
        ...
        for (int j = 0; j < entryCount; j++) {
            ...
            final float drawRadius = mChart.getDrawRadius(j);
            final float userInnerRadius = drawInnerArc
                    ? drawRadius * (mChart.getHoleRadius() / 100.f)
                    : 0.f;

            float sliceAngle = drawAngles[j];
            float innerRadius = userInnerRadius;

            RectF circleBoxFill = mChart.getCircleBoxWithRadius(drawRadius);
            ...
            if ((Math.abs(e.getY()) > Utils.FLOAT_EPSILON)) {
                ...
                if (!mChart.needsHighlight(j)) {
                    ...
                    final float sliceSpaceAngleOuter = visibleAngleCount == 1 ?
                            0.f :
                            sliceSpace / (Utils.FDEG2RAD * drawRadius);
                    ...
                    float arcStartPointX = center.x + drawRadius * (float) Math.cos(startAngleOuter * Utils.FDEG2RAD);
                    float arcStartPointY = center.y + drawRadius * (float) Math.sin(startAngleOuter * Utils.FDEG2RAD);

                    if (sweepAngleOuter >= 360.f && sweepAngleOuter % 360f <= Utils.FLOAT_EPSILON) {
//                         Android is doing "mod 360"
                        mPathBuffer.addCircle(center.x, center.y, drawRadius, Path.Direction.CW);
                    } else {
    ...
    protected void drawOuterDataSet(Canvas c, IPieDataSet dataSet) {
        ...
        for (int j = 0; j < entryCount; j++) {
            ...
            final float drawRadius = mChart.getRadius();
            final float trueRadius = mChart.getRadius();
            final float userInnerRadius = drawInnerArc
                    ? drawRadius * (mChart.getHoleRadius() / 100.f)
                    : 0.f;
            ...
            if ((Math.abs(e.getY()) > Utils.FLOAT_EPSILON)) {
                ...
                if (!mChart.needsHighlight(j)) {
                    ...
                    mRenderPaint.setColor(dataSet.getColor(j));
                    mRenderPaint.setAlpha(mOuterSpaceAlpha);

                    final float sliceSpaceAngleOuter = visibleAngleCount == 1 ?
                            0.f :
                            sliceSpace / (Utils.FDEG2RAD * trueRadius);
                    final float startAngleOuter = rotationAngle + (angle + sliceSpaceAngleOuter / 2.f) * phaseY;
                    float sweepAngleOuter = (sliceAngle - sliceSpaceAngleOuter) * phaseY;
                    if (sweepAngleOuter < 0.f) {
                        sweepAngleOuter = 0.f;
                    ...
                    float arcStartPointX = center.x + trueRadius * (float) Math.cos(startAngleOuter * Utils.FDEG2RAD);
                    float arcStartPointY = center.y + trueRadius * (float) Math.sin(startAngleOuter * Utils.FDEG2RAD);

                    if (sweepAngleOuter >= 360.f && sweepAngleOuter % 360f <= Utils.FLOAT_EPSILON) {
//                         Android is doing "mod 360"
                        mPathBuffer.addCircle(center.x, center.y, trueRadius, Path.Direction.CW);
                    } else {
                        mPathBuffer.moveTo(arcStartPointX, arcStartPointY);
                        mPathBuffer.arcTo(
                                circleBoxFill,
    ...
    public void drawValues(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = 0; j < entryCount; j++) {
                ...
                float vx = labelRadius / 1.5f * sliceXBase + center.x;
                float vy = labelRadius / 1.5f * sliceYBase + center.y;

                mEntryLabelsPaint.setTextAlign(Paint.Align.CENTER);

                if (j < data.getEntryCount() && entry.getLabel() != null) {
                    @SuppressLint("DefaultLocale")
                    String percentage = String.format("%.1f", (mChart.getDrawRadii(j) * 100f)) + "%";
                    Pair<Float, Float> pair = mChart.getDrawRadiiPair(j);
                    @SuppressLint("DefaultLocale")
                    String division = String.format("%.0f/%.0f", pair.first, pair.second);
                    drawEntryInnerLabel(c, percentage, vx, vy);
                    drawEntryInnerLabel(c, division, vx, vy + lineHeight);
                }
    ...
    protected void drawEntryInnerLabel(Canvas c, String label, float x, float y) {
        Paint p = new Paint(Paint.ANTI_ALIAS_FLAG);
        p.setColor(Color.WHITE);
        p.setTextAlign(Paint.Align.CENTER);
        p.setTextSize(Utils.convertDpToPixel(13f));
        p.setColor(mChart.getInnerLabelColor());
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (int i = 0; i < indices.length; i++) {
            ...
            final float radius = mChart.getDrawRadius(index - 1);
            final float userInnerRadius = drawInnerArc
